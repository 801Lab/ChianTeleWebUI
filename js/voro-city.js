...
function change(v){
	var str="0";
	for(i=0;i<v;i++){
		str+=("_"+(document.getElementById(s_1[i]).selectedIndex-1));
	};
	var ss=document.getElementById(s_1[v]);
	with(ss){
		length = 0;
		options[0]=new Option(opt0[v],opt0[v]);
		// console.log(options[1])
		if(v && document.getElementById(s_1[v-1]).selectedIndex>0 || !v){
			if(dsy.Exists(str)){
				ar = dsy.Items[str];
...
var s_1=["s_province1","s_city1"];//三个select的name
var s = ['s_province','s_city']
var opt0 = ["省份","地级市"];//初始值
var opt1 = ["省份","地级市"];
function _init_area(){  //初始化函数
	for(i=0;i<s_1.length-1;i++){
		document.getElementById(s_1[i]).onchange=new Function("change("+(i+1)+")");
	}
	change(0);
}
function _init_area1(){  //初始化函数
	for(i=0;i<s.length-1;i++){
		document.getElementById(s[i]).onchange=new Function("change1("+(i+1)+")");
	}
	change1(0);
}
...
dsy.add("0_15_0",["市中区","历下区","槐荫区","天桥区","历城区","长清区","章丘市","平阴区","济阳县","商河县","莱芜区","钢城区"]);
...
dsy.add("0_15",["济南市","青岛市","聊城市","德州市","东营市","淄博市","潍坊市","烟台市","威海市","日照市","临沂市","枣庄市","济宁市","泰安市","滨州市","菏泽市"]);
